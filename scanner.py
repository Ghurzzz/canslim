...
def canslim_score(ticker, info, hist, market_ok=True):
    ...
    # 1. EPS QoQ [ZORUNLU] - Ceyreklik EPS buyumesi >= %20
    v = safe('earningsQuarterlyGrowth')
    if v is not None:
        pct = round(v*100, 1)
        add('eps_qoq', f'{pct:+.1f}%', pct >= 20, '>=20%')
    else:
        # Fallback: yillik EPS buyumesi
    ...
    # 2. Rev Growth [ONEMLI] - Gelir buyumesi >= %15
    ...
    # 3. ROE [ONEMLI] - Oz sermaye getirisi >= %15
    ...
    # 4. Gross Margin [ONEMLI] - Brut marjin >= %40
    v = safe('grossMargins')
    if v is not None:
        pct = round(v*100, 1)
    ...
    # 5. Net Margin [DESTEK] - Net marjin >= %10
    ...
    # 6. D/E [DESTEK] - Borc/Ozkaynak <= 1.0
    # yfinance debtToEquity: 150 = 1.5 D/E orani
    v = safe('debtToEquity')
    if v is not None:
        val = round(v / 100, 2)
    ...
    # 7. Current Ratio [DESTEK] - Likidite orani >= 1.5
    v = safe('currentRatio')
    if v is not None:
    ...
    # 9. Market Cap [DESTEK] - Piyasa degeri >= 1 milyar
    v = safe('marketCap')
    if v is not None:
    ...
    # 10. 52H Yakinlik [ONEMLI] - 52 haftalik zirveye yakinlik >= %75
    ...
    # 11. SMA50 [ONEMLI] - Fiyat SMA50 uzerinde
    if len(closes) >= 50:
    ...
    # 12. SMA200 [ZORUNLU] - Fiyat SMA200 uzerinde
    if len(closes) >= 200:
    ...
    # 13. Rel Vol [DESTEK] - Goreceli hacim >= 0.8x
    # Son 5 gunun ortalama hacmini kullan (daha stabil)
    if len(hist) >= 20:
        vols = hist['Volume'].dropna()
        avg20 = float(vols.tail(20).mean())
        avg5  = float(vols.tail(5).mean())  # Son 5 gun ortalamasi
        rv = round(avg5 / avg20, 2) if avg20 > 0 else 0
        add('rel_vol', f'{rv}x (5g/20g)', rv >= 0.8, '>=0.8x')
    else:
        add('rel_vol', 'Veri yetersiz', False, '>=0.8x', False)

    # 14. Avg Vol [DESTEK] - Ortalama hacim >= 500K
    ...
    # 15. Inst Own [DESTEK] - Kurumsal sahiplik >= %40
    v = safe('institutionsPercentHeld')
    if v is not None:
        pct = round(v * 100, 1)
        add('inst_own', f'{pct:.1f}%', pct >= 40, '>=40%')
    else:
        # institutionHoldingsPercent fallback
        v2 = safe('institutionHoldingsPercent')
        if v2 is not None:
            pct = round(v2 * 100, 1)
            add('inst_own', f'{pct:.1f}%', pct >= 40, '>=40%')
        else:
            add('inst_own', 'Veri yok', True, '>=40%', False)  # Veri yok = gecti say (sert degil)

    # 16. M Kriteri [ZORUNLU] - Piyasa gucluyse al
    add('market', 'Guclu' if market_ok else 'Zayif', market_ok, 'Guclu')

    # Agirlikli skor hesapla
    weighted = sum(c[3] for c in CANSLIM_CRITERIA if cr_res.get(c[0], {}).get('passed'))
    raw      = sum(1     for c in CANSLIM_CRITERIA if cr_res.get(c[0], {}).get('passed'))
...
def run_canslim_screener(market_data, watchlist, portfolio):
    ...
    for ticker in all_tickers:
        try:
            ...
            weighted_score, raw_score, criteria_results = canslim_score(ticker, info, hist, market_ok)
            ...
            results.append({
                ...
                'pct': round(weighted_score/MAX_WEIGHTED_SCORE*100),
                'criteria': criteria_results,
                'sector': sector,
                'in_watchlist': ticker in watchlist,
                'in_portfolio': ticker in portfolio,
                'passed': weighted_score >= 19  # ~%70 ağırlıklı eşik
            })
        except Exception as e:
            continue
    ...
    print(f'  {len([r for r in results if r["passed"]])} hisse CANSLIM geçti (ağırlıklı ≥%70)')
    return results
