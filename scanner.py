...
def canslim_score(ticker, info, hist, market_ok=True):
    """16 CANSLIM kriteri için skor hesapla"""
    score = 0
    details = {}
    ...
    # 1. EPS QoQ >= 20%
    eps_qoq = safe('earningsQuarterlyGrowth')
    if eps_qoq is not None:
        eps_qoq_pct = eps_qoq * 100
        details['EPS QoQ'] = round(eps_qoq_pct, 1)
        if eps_qoq_pct >= 20: score += 1
    
    # 2. Revenue Growth >= 15%
    rev_growth = safe('revenueGrowth')
    if rev_growth is not None:
        rev_pct = rev_growth * 100
        details['Rev Growth'] = round(rev_pct, 1)
        if rev_pct >= 15: score += 1
    ...
    if gm is not None:
        gm_pct = gm * 100
        details['GrossMargin'] = round(gm_pct, 1)
        if gm_pct >= 40: score += 1
    
    # 5. Net Margin >= 10%
    nm = safe('profitMargins')
    if nm is not None:
        nm_pct = nm * 100
        details['NetMargin'] = round(nm_pct, 1)
        if nm_pct >= 10: score += 1
    
    # 6. Debt/Equity <= 1.0
    de = safe('debtToEquity')
    if de is not None:
        de_ratio = de / 100  # yfinance 0-100 scale
        details['D/E'] = round(de_ratio, 2)
        if de_ratio <= 1.0: score += 1
    
    # 7. Current Ratio >= 1.5
    cr = safe('currentRatio')
    if cr is not None:
        details['CurrentRatio'] = round(cr, 2)
        if cr >= 1.5: score += 1
    
    # 8. P/E <= 60
    pe = safe('trailingPE')
    if pe is not None and pe > 0:
        details['P/E'] = round(pe, 1)
        if pe <= 60: score += 1
    ...
    # 10. 52W High yakınlığı >= 85% (zirveye yakın)
    high52 = safe('fiftyTwoWeekHigh')
    if high52 and price > 0:
        pct_of_high = (price / high52) * 100
        details['52W%'] = round(pct_of_high, 1)
        if pct_of_high >= 75: score += 1  # biraz esnet
    ...
    # 13. Relative Volume >= 1.2
    if len(hist) >= 20:
        vols = hist['Volume'].dropna()
        avg_vol = float(vols.tail(20).mean())
        last_vol = float(vols.iloc[-1])
        rel_vol = round(last_vol / avg_vol, 2) if avg_vol > 0 else 0
        details['RelVol'] = rel_vol
        if rel_vol >= 0.8: score += 1  # biraz esnet
    
    # 14. Avg Volume >= 500K
    avg_v = safe('averageVolume')
    if avg_v is not None:
        details['AvgVol'] = int(avg_v)
        if avg_v >= 500000: score += 1
    
    # 15. Institutional Ownership >= 40%
    inst_own = safe('institutionsPercentHeld')
    if inst_own is not None:
        inst_pct = inst_own * 100
        details['InstOwn'] = round(inst_pct, 1)
        if inst_pct >= 40: score += 1
    
    # 16. M Kriteri (piyasa trendi) - market_ok parametresi
    details['MarketOK'] = market_ok
    if market_ok: score += 1
    
    return score, details

def run_canslim_screener(market_data, watchlist, portfolio):
    """CANSLIM screener - tüm tech universe + watchlist/portfolio"""
    ...
    # Tüm hisseler: universe + watchlist + portfolio (tekrarsız)
    ...
    for ticker in all_tickers:
        try:
            ...
            score, details = canslim_score(ticker, info, hist, market_ok)
            if score is None:
                continue
    ...
    print(f'  {len([r for r in results if r["passed"]])} hisse CANSLIM geçti (11/16+)')
    return results
